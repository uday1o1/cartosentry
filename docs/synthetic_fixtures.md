...
Each fixture records generator version `1.0.2`, its frozen synthetic family identifier, its seed, independent analytic source velocity, analytic truth, and a content-derived fixture identifier.
The fixture-set manifest binds every file by SHA-256 and also binds the frozen split manifest by SHA-256.
The same split-derived renderer can generate the exact 12-family `threshold_calibration` set without committing those generated bytes.
Callers select an ordinary partition, but the family identifiers, seeds, family count, and recorded partition come only from the immutable split manifest.
Any mismatch between a family-set identity and its declared partition fails before generation.
